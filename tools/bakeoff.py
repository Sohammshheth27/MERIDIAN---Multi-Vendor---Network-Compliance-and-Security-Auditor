...
from meridian.ai.interpret import OllamaInterpreter
from meridian.frameworks import load_all
from meridian.nlp import NlpMatcher, corpus_from_packs, corpus_from_stig
...
def main() -> None:
    reg = load_all()
    corpus = corpus_from_packs(str(_ROOT / "packs"), str(_ROOT / "samples")) + corpus_from_stig(reg)
    ...
    print("            (this is the false-PASS path -- lower is better)")

    with open(_ROOT / "tools" / "bakeoff_results.json", "w", encoding="utf-8") as fh:
