...
Consequences enforced elsewhere:
  ...
  * The examples.jsonl generator must exclude reference/cis_benchmarks/
...
def load(cis_dir: str | Path, *, latest_only: bool = True) -> Catalog:
    ...
    product family, which is what a scan should cite. The chosen version must
    be pinned into the assessment record.
    """
    root = Path(cis_dir)
