...
The design keeps Parser RAG ("how is this vendor's config structured?") separate
...
class AiSecurityKB(BaseModel):
    ...
    def poisoning_techniques(self) -> list[AtlasTechnique]:
        """Relevant to a bad approval poisoning the registry."""
        return [
            t for t in self.techniques.values()
...
def load_atlas(stix_path: str | Path) -> AiSecurityKB:
    ...
    kb.ai_rmf_functions = {
        "GOVERN": "Roles, separation of duties, approval policy",
...
# Guardrail -> framework coverage matrix.
