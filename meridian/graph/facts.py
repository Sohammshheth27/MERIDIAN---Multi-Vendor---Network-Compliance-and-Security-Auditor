"""Security facts computed from the object graph.

These are the `fact.*()` primitives: exposure must be COMPUTED from the
...
def admin_mgmt_public_exposure(graph: ObjectGraph, resolver: Resolver | None = None):
    """fact.admin_mgmt_public_exposure().

    An administrative service reachable from the whole internet. This is the
    exposure factor feeding the risk formula: severity alone would call
