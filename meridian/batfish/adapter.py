...
def analyse(config_path, platform: str, *, snapshot_dir=None,
    ...
    import tempfile
    root = Path(snapshot_dir or tempfile.mkdtemp(prefix="meridian-bf-"))
    cfg = root / "configs"
...
def cross_check(hygiene_report, batfish_report) -> CrossCheck:
    ...
    "Only MERIDIAN" may be a false positive of ours; "only Batfish" is a gap in
    ...
    out.only_meridian = sorted(ours - theirs)[:50]
