...
The design already demands Parser RAG and Security RAG stay separate. This is a
third corpus, isolated from both, and :func:`assert_not_parser_corpus` exists so
a future refactor cannot quietly merge them.

Retrieval is TF-IDF, not embeddings: this corpus is small, the
...
class GovernanceKB:
    ...
    def build(cls, ai_kb: AiSecurityKB) -> "GovernanceKB":
        ...
        for i, g in enumerate(GUARDRAIL_COVERAGE):
            docs.append(
                GovernanceDoc(
                    doc_id=f"MERIDIAN.GUARD.{i:02d}",
                    source="MERIDIAN guardrails",
                    title=g["guardrail"],
                    text=(
    ...
    def coverage_matrix(self) -> str:
        """Our guardrails down the side, frameworks across the top."""
        rows = ["GUARDRAIL COVERAGE", ""]
...
def assert_not_parser_corpus(records: list[dict]) -> None:
    """Tripwire for the examples.jsonl generator.
    ...
    for r in records:
        if r.get("corpus") == CORPUS_TAG:
            raise ValueError(
                ...
                "containing configuration data -- see meridian/knowledge/governance.py."
            )
