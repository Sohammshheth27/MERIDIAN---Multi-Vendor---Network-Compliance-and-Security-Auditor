...
compiled. Same architecture, one box simplified.
...
class Mapping(BaseModel):
    ...
    parent: str | None = Field(
        ...
        description="Parent-line regex. Its presence makes the field SCOPED by "
                    "that parent -- `transport input ssh` under "
...
def derive_admin_ports_open_to_world(doc: JsonDocument, params: dict):
    """fact.admin_mgmt_public_exposure() for cloud.
...
def apply_json_pack(
    ...
    for m in pack.mappings:
        ...
        if m.scope:
            # Every field is scoped. Three security groups have
        ...
        if target_type == "list":
            ...
            obs = (
                ...
                if coerced is not None
                # A value we could not coerce is UNPARSED -> UNKNOWN,
                # never a guess.
