...
about 50 ms, and it is the reason a stalled or cold Ollama does not stop the
system returning a result.
"""
...
def corpus_from_packs(pack_dir: str, sample_dir: str | None = None) -> list[Example]:
    """The RAG corpus is a by-product of work already done.

    Run every mapping-pack regex over the sample configs; every match yields a
    (real line, field, value) triple. Using real lines matters: deriving
    pseudo-lines by stripping regex syntax produced
...
def corpus_from_stig(registry, limit_per_field: int = 40) -> list[Example]:
    """STIG fix text is simultaneously rule content AND
