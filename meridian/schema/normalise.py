...
----------------------
Guardrail 5 validates types with Pydantic: ``ssh.version`` must be
...
be normalised we return None and the caller records UNKNOWN. The critical
rule is: never guess.
"""
import re
...
def normalise(raw: object, as_type: str) -> object | None:
    ...
    UNKNOWN rather than guessing: UNKNOWN always beats a wrong PASS.
    """
