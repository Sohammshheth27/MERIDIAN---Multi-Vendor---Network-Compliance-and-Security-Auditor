...
configuration (read-only, meridian/collect/live.py), assesses it exactly as an
...
class Monitor:
    ...
    def run(self, job_id: str, ingest, uploads: Path) -> dict:
        ...
        # meridian.diff re-exports, not the module; import the module by name.
    ...
    def start(self, ingest, uploads: Path, *, tick_seconds: int = 30) -> None:
        ...
        self._thread = threading.Thread(target=loop, name="meridian-monitor", daemon=True)
        self._thread.start()
