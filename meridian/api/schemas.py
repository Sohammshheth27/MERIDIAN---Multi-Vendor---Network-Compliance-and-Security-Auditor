...
class CollectIn(BaseModel):
    """Live collection request (meridian/collect/live.py).
...
class MonitorIn(CollectIn):
    """A scheduled re-collection job (meridian/collect/monitor.py). The password
...
class RecordAccountingOut(BaseModel):
    """Accounting: total == parsed + unknown, and mapped is a subset of parsed."""
...
class AuthStatusOut(BaseModel):
    ...
    pairing_open: bool = Field(
        ...
        description="Whether the page should show the pairing QR. Normally "
                    ...
                    "engine runs with MERIDIAN_SHOW_PAIRING=1.")
    locked_seconds: int = 0
