"""HTTP surface for MERIDIAN.

    uvicorn meridian.api.app:app --reload --port 8000
...
# now explicit:
#
#   MERIDIAN_API_TOKEN set    -> every API call must carry
#                            `Authorization: Bearer <token>`.
#   MERIDIAN_API_TOKEN unset  -> the API answers THIS machine only (loopback), so
#                            a demo still needs no setup but is not exposed
#                            to the network by default.
#   MERIDIAN_CORS_ORIGINS     -> comma-separated browser origins allowed to call
...
def _origins() -> list[str]:
    raw = _os_env("MERIDIAN_CORS_ORIGINS")
...
def _console_auth_on() -> bool:
    ...
    """
    return (_os_env("MERIDIAN_CONSOLE_AUTH") or "1").strip().lower() not in (
        "0", "false", "off", "no")
...
async def _require_access(request: Request, call_next):
    ...
    token = _os_env("MERIDIAN_API_TOKEN")
        ...
        if host not in _LOOPBACK:
            return JSONResponse(
                {"detail": "this Meridian instance answers the local machine only; "
                           "set MERIDIAN_API_TOKEN to allow remote clients"},
                status_code=403)
    return await call_next(request)
...
# store.py). MERIDIAN_DATA_DIR moves the data directory; it defaults to ./data,
# which is git-ignored because it holds customer configurations.
import os as _os

from .store import AssessmentStore

_DATA_DIR = Path(_os.environ.get("MERIDIAN_DATA_DIR")
...
def auth_status(request: Request):
    ...
    return AuthStatusOut(
        ...
        # separate question, so the pairing switch does not make the engine
...
def auth_enroll(request: Request):
    ...
    MERIDIAN_SHOW_PAIRING=1 holds it open for demonstrations. See
...
def fleet_csv():
    ...
    return Response(buf.getvalue(), media_type="text/csv",
                    headers={"Content-Disposition":
                             'attachment; filename="meridian_fleet.csv"'})
...
def get_report(aid: str, format: str = Query("pdf", pattern="^(pdf|html)$"),
    ...
    out = _UPLOADS / f"MERIDIAN_Report_{safe}_{aid}{suffix}.pdf"
    try:
    ...
    return FileResponse(str(out), media_type="application/pdf",
                        filename=out.name,
                        headers={"X-MERIDIAN-Report-SHA256": signed["sha256"],
                                 "X-MERIDIAN-Signature": signed["signature"]})
...
def report_signing_key():
    ...
    offline with this and the X-MERIDIAN-Signature header; no secret needed."""
...
def get_extended(aid: str):
    ...
    meridian/extended/model.py for why.
    """
...
def attack_coverage():
    ...
    def build():
        ...
        ids += (sorted(CHECKS) + [f"MERIDIAN-X-VPN-00{i}" for i in range(1, 6)]
                + [CHECK_ID])
        return coverage(ids)
