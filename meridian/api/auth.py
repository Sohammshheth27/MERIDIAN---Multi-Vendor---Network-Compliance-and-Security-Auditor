...
satisfy the controls it audits for. Account lockout is MERIDIAN-EXT-013.
Administrative MFA is MERIDIAN-EXT-014. Failed-login logging is MERIDIAN-EXT-021.
...
#: bcrypt hash of the default credential, cost 12. Override in any real
#: deployment with MERIDIAN_ADMIN_PASSWORD_HASH.
DEFAULT_PASSWORD_HASH = "$2b$12$EmXVZKnEWbyx19OsDwDGge5rJ1KttcH4/vBbgZoqmJpJmHS/Y53sq"

ISSUER = "MERIDIAN"
SESSION_HOURS = 8

#: MERIDIAN-EXT-013 (account lockout) applied to ourselves.
MAX_FAILURES = 5
LOCKOUT_SECONDS = 300
...
def pairing_always_open() -> bool:
    ...
    """
    return (os.environ.get("MERIDIAN_SHOW_PAIRING") or "").strip().lower() in {
        "1", "true", "yes", "on"}


def _data_dir() -> Path:
    d = Path(os.environ.get("MERIDIAN_DATA_DIR")
...
# ------------------------------------------------------------- credentials
def admin_user() -> str:
    return os.environ.get("MERIDIAN_ADMIN_USER") or DEFAULT_USER


def _password_hash() -> bytes:
    return (os.environ.get("MERIDIAN_ADMIN_PASSWORD_HASH")
...
def issue_session(username: str) -> str:
    ...
    return URLSafeTimedSerializer(_session_key(), salt="meridian-console").dumps(
        {"u": username})
...
def verify_session(token: str) -> str | None:
    ...
    if not token:
        return None
    try:
        data = URLSafeTimedSerializer(_session_key(), salt="meridian-console").loads(
