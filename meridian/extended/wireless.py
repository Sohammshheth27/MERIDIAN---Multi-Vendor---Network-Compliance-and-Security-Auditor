...
CHECKS = {
    "MERIDIAN-X-WLAN-001": ("Wireless networks must not be open (unencrypted)",
                        "high", ["AC-18.1", "SC-8"],
                        ...
                        "guest networks."),
    "MERIDIAN-X-WLAN-002": ("Deprecated wireless security (WEP, WPA1, TKIP) must "
                        "not be used", "high", ["AC-18.1", "SC-13"],
                        "WEP keys are recoverable in minutes, and WPA1/TKIP "
                        "are deprecated by the Wi-Fi Alliance."),
    "MERIDIAN-X-WLAN-003": ("Protected Management Frames must be enabled",
                        "medium", ["AC-18.1", "SC-8"],
    ...
    "MERIDIAN-X-WLAN-004": ("Guest wireless clients must be isolated from each "
                        ...
                        "attacking the rest."),
    "MERIDIAN-X-WLAN-005": ("Wireless pre-shared keys must not be stored in "
...
def check_network(n: WirelessNetwork) -> list[ExtendedFinding]:
    ...
    if n.security == "open":
        ...
        out.append(_f("MERIDIAN-X-WLAN-001", "FAIL", n, why, n.security,
                      "WPA2/WPA3 or OWE", sec_ev))
    else:
        out.append(_f("MERIDIAN-X-WLAN-001", "PASS", n,
    ...
    if "tkip" in n.ciphers:
        bad.append("TKIP")
    if bad:
        out.append(_f("MERIDIAN-X-WLAN-002", "FAIL", n,
                      f"deprecated security in use: {', '.join(bad)}",
                      bad, "none", [n.evidence.get("deprecated", hdr)]))
    elif n.security == "open":
        out.append(_f("MERIDIAN-X-WLAN-002", "NOT_APPLICABLE", n,
                      "the network is open, which MERIDIAN-X-WLAN-001 reports; "
                      "there is no cipher to judge", ev=sec_ev))
    else:
        out.append(_f("MERIDIAN-X-WLAN-002", "PASS", n,
                      f"no WEP, WPA1 or TKIP{basis}", sorted(n.ciphers),
                      "AES only", sec_ev))

    # 003 PMF
    if n.security in ("open",):
        out.append(_f("MERIDIAN-X-WLAN-003", "NOT_APPLICABLE", n,
                      "PMF protects encrypted associations; this network "
                      "has none", ev=sec_ev))
    elif n.pmf:
        out.append(_f("MERIDIAN-X-WLAN-003", "PASS", n, f"PMF is {n.pmf}",
                      n.pmf, "optional or mandatory",
                      [n.evidence.get("pmf", sec_ev[0])]))
    else:
        out.append(_f("MERIDIAN-X-WLAN-003", "FAIL", n,
    ...
    if not n.guest:
        ...
        out.append(_f("MERIDIAN-X-WLAN-004", "FAIL", n,
    ...
    if "psk" not in n.akm and "sae" not in n.akm:
        out.append(_f("MERIDIAN-X-WLAN-005", "NOT_APPLICABLE", n,
                      "the network does not use a pre-shared key", ev=[hdr]))
    elif n.cleartext_psk:
        out.append(_f("MERIDIAN-X-WLAN-005", "FAIL", n,
        ...
        out.append(_f("MERIDIAN-X-WLAN-005", "PASS", n,
