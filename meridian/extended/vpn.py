...
def check_tunnel(t: VpnTunnel) -> list[ExtendedFinding]:
    ...
    cid, title = "MERIDIAN-X-VPN-001", "IPsec tunnels must use perfect forward secrecy"
    ...
    # --- VPN-002: anti-replay -----------------------------------------------
    cid, title = "MERIDIAN-X-VPN-002", "IPsec anti-replay protection must be enabled"
    ...
    cid, title = "MERIDIAN-X-VPN-003", "IKE and IPsec SA lifetimes must be bounded"
    nist, sev = ["SC-12"], "low"
...
def assess_vpn(da) -> DomainResult:
    ...
    notes = ["Algorithm strength (MERIDIAN-X-VPN-005) is UNKNOWN by design: the "
