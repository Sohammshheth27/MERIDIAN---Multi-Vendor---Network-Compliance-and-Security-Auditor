...
from meridian.frameworks.selection import framework_coverage
...
def test_stig_judges_the_idle_timeout_by_its_own_five_minutes(tmp_path):
    ...
    t = next(f for f in da.assessment.findings if f.control_id == "MERIDIAN-TIME-001")
    assert t.state.value == "PASS", "MERIDIAN's own threshold is 10 minutes"
    rows = {r["framework"]: r for r in framework_coverage(da.assessment.findings,
                                                          da.identity.platform)}
    assert "MERIDIAN-TIME-001" in rows["stig"]["profiled_controls"], "STIG says 5"
    assert "MERIDIAN-TIME-001" not in rows["nist_800_53"]["profiled_controls"]
