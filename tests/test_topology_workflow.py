...
The last three capabilities the comparable products had and MERIDIAN did not.
...
from meridian.graph.model import ObjectGraph, SecurityRule
from meridian.logs import corroborate, parse, parse_line
from meridian.pipeline import assess
from meridian.topology import Fabric, Interface
from meridian.topology.interfaces import from_indented
from meridian.workflow import Register, deletion_candidates, review
...
def test_a_redacted_upload_still_yields_its_interfaces():
    ...
    from meridian.topology.interfaces import extract
...
def test_unused_and_unowned_ranks_above_merely_unowned():
    from meridian.graph.hygiene import analyse
...
def test_an_expired_certification_is_high_severity():
    reg = Register(path=None)
    reg.certify("fw1", "r0", owner="operator", period_days=30,
                when=date.today() - timedelta(days=90))
    f = [x for x in review(_DA(_graph_with(1, hits=5)), reg)
         if x.rule_id == "r0"]
    assert f and f[0].kind == "expired" and f[0].owner == "operator"
