...
WE named wrongly. Ruling on it either way asserts a fact from silence:
  ...
  MERIDIAN-EXT-039  security.ips.enabled   <- uuidIpsObjEnable  (0 records)
      FAILED, claiming IPS is off on a device whose IPS state was never read.
  MERIDIAN-AAA-002  authentication.local_accounts <- adminName* (0 records)
...
DEAD = {
    "MERIDIAN-CAT-006": "firewall.acls",            # policyNgName*
    "MERIDIAN-EXT-039": "security.ips.enabled",     # uuidIpsObjEnable
    "MERIDIAN-EXT-040": "security.gav.enabled",     # uuidGavObjEnable
    "MERIDIAN-AAA-002": "authentication.local_accounts",   # adminName*
}
...
def test_the_false_pass_is_gone(sw):
    ...
    f = next(x for x in sw.assessment.findings if x.control_id == "MERIDIAN-AAA-002")
...
def test_a_declared_platform_default_still_decides(sw):
    ...
    f = next(x for x in sw.assessment.findings
             if x.control_id == "MERIDIAN-CLD-004")
