"""Who may call the API (meridian/api/app.py, the security middleware).
...
def test_with_a_token_every_api_call_needs_it(monkeypatch):
    monkeypatch.setenv("MERIDIAN_API_TOKEN", "s3cret-token")
    c = TestClient(app)
...
def test_without_a_token_only_this_machine_is_answered(monkeypatch):
    ...
    assert r.status_code == 403 and "MERIDIAN_API_TOKEN" in r.text
