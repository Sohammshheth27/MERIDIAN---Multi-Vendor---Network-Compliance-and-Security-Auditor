...
from meridian.engine.rules import CIS_CROSSWALK, cis_benchmark, load_rules
from meridian.frameworks.selection import (FRAMEWORKS, framework_coverage,
                                       normalise)
...
def test_a_requirement_is_met_only_when_every_citing_check_passes():
    from meridian.frameworks.selection import requirement_state as rs
...
def test_undecided_checks_are_not_counted_as_passes():
    from meridian.frameworks.selection import requirement_satisfaction as sat
...
def test_the_report_states_the_selection_and_the_per_framework_result():
    from meridian.report import build_report
...
def _benchmark_ids(titles: set[str]) -> dict[str, set[str]]:
    ...
    import meridian.frameworks.cis as cis_mod
...
def test_cis_numbers_reach_the_findings():
    ...
    ssh2 = next(f for f in da.assessment.findings if f.control_id == "MERIDIAN-SSH-002")
    per = (_crosswalk().get("map") or {}).get("MERIDIAN-SSH-002", {})
