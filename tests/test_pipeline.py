...
from meridian.engine.fingerprint import Fingerprint
from meridian.engine.risk import (BASE, EXPOSURE_INTERNET, Risk, band_of,
                              score_assessment, score_finding)
from meridian.pipeline import assess, assess_many, load_packs, select_pack
from meridian.schema.enums import ResultState, Severity
...
# ------------------------------------------------------------- remediation
from meridian.engine.remediate import (IMPACT_DISABLES_HTTP, PHASE_DISABLE,
                                   PHASE_ENABLE, Plan, Step, build_plan)
from meridian.engine.rules import load_rules
...
class _DA:
    def __init__(self, findings, platform="cisco_iosxe_router"):
        from meridian.pipeline import DeviceIdentity
...
def test_disable_steps_run_after_enable_steps():
    ...
    da = _DA([_finding("MERIDIAN-HTTP-001"), _finding("MERIDIAN-SSH-002",
    ...
    if "MERIDIAN-HTTP-001" in by_id and "MERIDIAN-SSH-002" in by_id:
        assert by_id["MERIDIAN-SSH-002"] < by_id["MERIDIAN-HTTP-001"]


def test_step_is_deferred_when_it_is_the_only_transport():
    """The lockout guard: do not cut the only way in."""
    ctrls = {c.id: c for c in load_rules("rules", platform="cisco_iosxe_router")}
    da = _DA([_finding("MERIDIAN-HTTP-001")])
    ...
    assert not any(s.control_id == "MERIDIAN-HTTP-001" for s in plan.steps)
...
def test_step_runs_with_a_warning_when_another_transport_exists():
    ...
    step = [s for s in plan.steps if s.control_id == "MERIDIAN-HTTP-001"][0]
    assert "confirm you are connected over" in step.lockout_warning


def test_script_carries_a_vendor_rollback_net():
    ctrls = {c.id: c for c in load_rules("rules", platform="cisco_iosxe_router")}
    plan = build_plan(_DA([_finding("MERIDIAN-HTTP-001")]),
...
def test_missing_remediation_is_reported_not_invented():
    ...
    da = _DA([_finding("MERIDIAN-NOT-A-REAL-CONTROL")])
    plan = build_plan(da, controls_by_id={}, sbm=_SBM({}))
    assert plan.unavailable == ["MERIDIAN-NOT-A-REAL-CONTROL"]
    assert not plan.steps


# --------------------------------------------------------- training queue
from meridian.training import build_queue, classify
from meridian.training.classify import (CAT_COSMETIC, CAT_EMPTY, CAT_OTHER,
                                    CAT_SECURITY, base_name)
from meridian.training.queue import _humanise
...
def test_asa_telnet_timeout_is_not_telnet_enabled():
    ...
    r = assess(ASA)
    f = [x for x in r.assessment.findings if x.control_id == "MERIDIAN-TEL-001"][0]
...
def test_local_only_aaa_is_not_external_authentication():
    ...
    r = assess(ASA)
    f = [x for x in r.assessment.findings if x.control_id == "MERIDIAN-EXT-015"][0]
...
def test_alternative_mappings_do_not_erase_each_other():
    ...
    import hashlib
    from meridian.readers import apply_indented_pack, load_indented
    from meridian.readers.pack import load_pack
    from meridian.schema.enums import ObservationState
...
def test_no_control_compares_a_field_against_an_impossible_type():
    ...
    passes. MERIDIAN-EXT-012 compared a bool against the string "password" and so
    could not fail on any device ever assessed."""
    from meridian.engine.rules import load_rules
    from meridian.schema.sbm import FIELD_TYPES
    dead = []
...
def test_disabled_feature_makes_subcontrols_not_applicable():
    ...
    r = assess(HARDENED_SW)
    f = [x for x in r.assessment.findings if x.control_id == "MERIDIAN-EXT-023"][0]
...
def test_precondition_never_fires_on_an_unmapped_field():
    ...
    snmp = [x for x in r.assessment.findings if x.control_id == "MERIDIAN-SNMP-002"][0]
