...
from meridian.engine.fingerprint import fingerprint_file
from meridian.pipeline import assess
from meridian.training import apply as training
from meridian.training import build_queue
...
def test_teaching_sonic_makes_it_assessable_with_real_verdicts(clean_packs):
    ...
    assert _state(da, "MERIDIAN-SNMP-002").evidence
...
def test_api_teaches_sonic_and_reassesses(clean_packs, client):
    ...
    snmp = next(f for f in after["findings"] if f["control_id"] == "MERIDIAN-SNMP-002")
...
def test_schema_fields_name_the_controls_they_drive(client):
    ...
    assert "MERIDIAN-SNMP-002" in fields["snmp.communities"]["controls"]
