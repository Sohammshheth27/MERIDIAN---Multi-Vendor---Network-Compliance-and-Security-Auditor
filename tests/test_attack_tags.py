...
from meridian.extended.cve import CHECK_ID as CVE_CHECK
from meridian.extended.wireless import CHECKS as WLAN_CHECKS
from meridian.frameworks.attack import (ATTACK_MAP, BLAST_ADMIN_TECHNIQUE, BUNDLE,
                                    coverage, load_techniques, tags_for)

bundle_only = pytest.mark.skipif(not BUNDLE.exists(), reason="ATT&CK bundle absent")

VPN_CHECKS = {f"MERIDIAN-X-VPN-00{i}" for i in range(1, 6)}
...
def test_tags_resolve_names_from_the_bundle(techniques):
    tags = tags_for("MERIDIAN-SNMP-002", techniques)
...
def test_an_unresolvable_tag_is_dropped_not_shown():
    ...
    assert tags_for("MERIDIAN-TEL-001", techniques={}) == []


def test_decorative_controls_stay_untagged():
    for cid in ("MERIDIAN-BAN-001", "MERIDIAN-EXT-028", "MERIDIAN-EXT-042"):
