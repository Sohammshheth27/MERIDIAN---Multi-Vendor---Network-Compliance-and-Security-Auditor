...
def test_asa_ssh_timeout_is_minutes_not_seconds(tmp_path: Path):
    ...
    f = _finding(da, "MERIDIAN-EXT-006")
    assert (f.observed, f.state.value) == (900, "FAIL")

    p.write_text(ASA.format(t=1))
    f = _finding(assess(str(p), redact=False, assessment_id="T-ASA-UNIT2"), "MERIDIAN-EXT-006")
...
def test_junos_connection_limit_is_not_a_login_attempt_limit(tmp_path: Path):
    ...
    assert _finding(da, "MERIDIAN-SSH-004").state.value == "UNKNOWN"
    assert _finding(da, "MERIDIAN-VTY-002").observed is None, \
        "connection-limit is not a source restriction either"


def test_thresholds_over_several_instances_need_every_one():
    from meridian.engine.operators import op_gte, op_lte
    from meridian.schema.enums import ResultState as S
...
def test_junos_idle_timeout_is_judged_per_class(tmp_path: Path):
    ...
    f = _finding(assess(str(p), redact=False, assessment_id="T-JUN-IT"), "MERIDIAN-TIME-001")
...
<class><name>audit</name><idle-timeout>60</idle-timeout></class>
    ...
    f = _finding(assess(str(x), redact=False, assessment_id="T-JUN-ITX"), "MERIDIAN-TIME-001")
...
def test_a_one_line_junos_block_is_read_not_dropped():
    ...
    from meridian.readers.braces import loads
...
def test_junos_retry_options_decide_the_lockout_control(tmp_path: Path):
    ...
    f = _finding(assess(str(p), redact=False, assessment_id="T-JUN-RO"), "MERIDIAN-SSH-004")
