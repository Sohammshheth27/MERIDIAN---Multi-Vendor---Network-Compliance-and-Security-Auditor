...
def auth(tmp_path, monkeypatch):
    ...
    MERIDIAN_DATA_DIR is redirected because the module persists the TOTP secret
    ...
    monkeypatch.setenv("MERIDIAN_CONSOLE_AUTH", "1")
    monkeypatch.delenv("MERIDIAN_API_TOKEN", raising=False)
    from meridian.api import auth as mod
...
def test_the_stored_credential_is_a_hash_not_the_password(auth):
    """The point of the whole exercise. `MERIDIAN-PLT-002` reports a device that
...
def test_repeated_failures_lock_the_account(auth):
    """MERIDIAN-EXT-013 is a control this product reports on. The console it ships
...
def test_the_api_token_still_works_for_scripts(client, monkeypatch):
    ...
    monkeypatch.setenv("MERIDIAN_API_TOKEN", "script-token")
...
def test_the_pairing_gate_is_shut_unless_it_is_opened_on_purpose(client, auth,
                                                                 monkeypatch):
    """MERIDIAN_SHOW_PAIRING is the only thing that reopens enrolment.

    The switch exists so the pairing step can be replayed during setup and
    testing. This pins the half that matters: with the variable absent or set
    to anything that is not an affirmative, the gate behaves exactly as it
    always did.
    """
    monkeypatch.delenv("MERIDIAN_SHOW_PAIRING", raising=False)
    r = client.post("/auth/login", json={"username": USER, "password": PASSWORD,
                                         "otp": _code(auth)})
    assert r.json()["ok"] is True
    assert client.get("/auth/enroll").status_code == 403
    ...
    for off in ("0", "no", "off", "", "maybe"):
        monkeypatch.setenv("MERIDIAN_SHOW_PAIRING", off)
        assert client.get("/auth/enroll").status_code == 403, off

    monkeypatch.setenv("MERIDIAN_SHOW_PAIRING", "1")
    assert client.get("/auth/enroll").status_code == 200
