...
from meridian.engine.fingerprint import fingerprint_file, fingerprint_json
from meridian.graph import azure_builder, gcp_builder
from meridian.graph.facts import (admin_mgmt_public_exposure, policy_broad_any_any,
                              sensitive_service_exposed)
from meridian.graph.reach import Query, ask
from meridian.pipeline import assess
from meridian.readers.json_reader import loads
...
def test_azure_internet_exposure_is_reported_with_its_json_path(az):
    f = next(x for x in az.assessment.findings if x.control_id == "MERIDIAN-CLD-001")
...
def test_azure_egress_is_not_passed_on_the_strength_of_the_defaults(az):
    ...
    f = next(x for x in az.assessment.findings if x.control_id == "MERIDIAN-CAT-009")
...
def test_cloud_platforms_get_honest_extended_answers(az, gcp):
    from meridian.extended.run import run_extended
