...
from meridian.paths import resolve_packs_dir
import pytest

from meridian.training import apply as ap
from meridian.training.apply import (ApprovalResult, _mapping_entry, approve,
...
def test_field_must_be_in_the_schema_whitelist():
    r = approve("someSetting", "not.a.real.field", "cisco_asa", "operator")
...
def test_unknown_platform_is_refused():
    r = approve("x", "logging.enabled", "no_such_platform", "operator")
...
def test_a_blocked_approval_leaves_nothing_behind(monkeypatch):
    ...
    def fake():
        ...
        if calls["n"] == 1:
            return {"cisco-asa-real": {"MERIDIAN-TEL-001": "PASS"}}
        return {"cisco-asa-real": {"MERIDIAN-TEL-001": "FAIL"}}

    monkeypatch.setattr(ap, "corpus_snapshot", fake)
    monkeypatch.setattr(ap, "_verified_expectations",
                        lambda: {"cisco-asa-real": {"MERIDIAN-TEL-001": "PASS"}})

    r = approve("telnet timeout", "management.telnet.enabled", "cisco_asa", "t")
    assert not r.accepted
    assert r.broken == ["cisco-asa-real:MERIDIAN-TEL-001"]
