...
def test_console_is_served_by_the_same_process():
    """One command, no build step: the console must be served by the engine
...
def test_landing_page_is_self_contained():
    ...
    Assets are self-hosted so the page does not depend on outbound network
    access, and the page that fails is the first one anybody sees.
    """
    r = client.get("/")
    assert r.status_code == 200
...
def test_the_schema_field_count_on_the_page_is_real():
    ...
    """
    from meridian.schema import FIELD_NAMES
...
def test_glass_never_dims_a_label_below_readable():
    ...
    number a reviewer has to squint at is a number they do not check.
    """
    def _srgb(c):
        c /= 255.0
