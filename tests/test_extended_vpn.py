...
from meridian.extended.model import ExtendedFinding
from meridian.extended.vpn import VpnTunnel, assess_vpn, check_tunnel
from meridian.pipeline import assess
from meridian.schema.evidence import EvidenceRef
...
def test_pfs_failures_are_the_tunnels_the_file_says(sw):
    ...
    got = sorted(f.scope for f in res.findings
                 if f.check_id == "MERIDIAN-X-VPN-001" and f.state == "FAIL")
    assert got == expected
...
def test_anti_replay_is_read_with_its_inverted_polarity(sw):
    ...
    got = sorted(f.scope for f in res.findings
                 if f.check_id == "MERIDIAN-X-VPN-002" and f.state == "FAIL")
    assert got == expected
...
def test_lifetime_bounds():
    ...
    assert _state(ok, "MERIDIAN-X-VPN-003") == "PASS"
    long_ike = check_tunnel(_tunnel(ike_lifetime_s=86_401))
    assert _state(long_ike, "MERIDIAN-X-VPN-003") == "FAIL"
    unbounded = check_tunnel(_tunnel(ipsec_lifetime_s=0))
    assert _state(unbounded, "MERIDIAN-X-VPN-003") == "FAIL", "0 means no limit"
...
def test_management_over_a_tunnel_goes_to_a_person_not_a_verdict():
    ...
    assert _state(check_tunnel(t), "MERIDIAN-X-VPN-004") == "MANUAL_REVIEW"


def test_missing_settings_are_unknown_not_pass():
    t = _tunnel(pfs=None, anti_replay=None)
    out = check_tunnel(t)
    assert _state(out, "MERIDIAN-X-VPN-001") == "UNKNOWN"
    assert _state(out, "MERIDIAN-X-VPN-002") == "UNKNOWN"
...
def test_every_nist_id_cited_exists_in_the_catalogue(sw):
    from meridian.frameworks.models import Framework
    from meridian.frameworks.registry import load_all
