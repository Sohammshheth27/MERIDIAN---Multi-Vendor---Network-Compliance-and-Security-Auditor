...
from meridian.batfish import SUPPORTED, analyse, cross_check, status, supports
from meridian.batfish.adapter import BatfishFinding, BatfishReport, CrossCheck
...
def test_cross_check_reports_nothing_when_batfish_was_not_consulted():
    ...
    assert not c.agreed and not c.only_meridian and not c.only_batfish


# --------------------------------------------------------------- consensus
def test_cross_check_reports_both_directions():
    """Only-MERIDIAN may be our false positive; only-Batfish is our gap. A
    ...
    assert c.only_meridian == ["ours-only"]
...
def test_two_independent_implementations_are_compared_on_one_device():
    from meridian.graph.hygiene import analyse as hygiene
