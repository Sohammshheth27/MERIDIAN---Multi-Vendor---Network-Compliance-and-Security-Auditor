...
def _session_packs(tmp_path_factory):
    ...
    MERIDIAN_PACKS_DIR (meridian/paths.py), so every read and write goes here.
    """
    ...
    prev = os.environ.get("MERIDIAN_PACKS_DIR")
    os.environ["MERIDIAN_PACKS_DIR"] = str(d)
    yield d
    if prev is None:
        os.environ.pop("MERIDIAN_PACKS_DIR", None)
    else:
        os.environ["MERIDIAN_PACKS_DIR"] = prev
...
def _session_store(tmp_path_factory):
    ...
    from meridian.api.store import AssessmentStore

    return AssessmentStore(tmp_path_factory.mktemp("meridian_data"))


@pytest.fixture(autouse=True)
def _isolated_audit_logs(tmp_path, monkeypatch, _session_store):
    from meridian.api import app as api_app
    from meridian.training import apply as training_apply
    ...
    monkeypatch.setenv("MERIDIAN_CONSOLE_AUTH", "0")
