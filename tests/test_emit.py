...
from meridian.engine.emit import _bool_like, _target, emit_sonicos, verify
...
def emitted():
    ...
    from meridian.pipeline import assess, load_packs, select_pack
...
def test_only_cited_records_change(emitted):
    ...
    from meridian.readers.sonicos_exp import loads
...
def test_a_dead_mapping_never_produces_a_change(emitted):
    ...
    da, em = emitted
    changed = {c.control_id for c in em.changes}
    for cid in ("MERIDIAN-EXT-039", "MERIDIAN-EXT-040", "MERIDIAN-CAT-006"):
...
def test_the_exp_envelope_is_the_inverse_of_the_reader():
    ...
    from meridian.engine.emit import as_exp, roundtrip_ok
    from meridian.readers.sonicos_exp import loads
