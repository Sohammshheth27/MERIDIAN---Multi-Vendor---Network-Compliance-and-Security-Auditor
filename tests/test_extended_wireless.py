...
from meridian.extended.wireless import (CHECKS, assess_wireless,
                                    networks_from_cisco)
from meridian.pipeline import assess
from meridian.readers.indented import loads
...
def test_open_guest_network_fails_and_names_the_captive_portal(wlc):
    _da, res = wlc
    f = _find(res, "MERIDIAN-X-WLAN-001", "Guest-WiFi")
    assert f.state == "FAIL"
...
def test_guest_isolation_fails_without_peer_blocking(wlc):
    _da, res = wlc
    assert _find(res, "MERIDIAN-X-WLAN-004", "Guest-WiFi").state == "FAIL"
    assert _find(res, "MERIDIAN-X-WLAN-004", "Secure-WPA3").state == "NOT_APPLICABLE"


def test_deprecated_security_is_caught(wlc):
    _da, res = wlc
    f = _find(res, "MERIDIAN-X-WLAN-002", "IoT-Devices")
    assert f.state == "FAIL"
...
def test_a_cleartext_psk_is_reported_but_never_reproduced(wlc):
    _da, res = wlc
    f = _find(res, "MERIDIAN-X-WLAN-005", "IoT-Devices")
    assert f.state == "FAIL"
...
def test_wpa3_carries_pmf_by_definition(wlc):
    _da, res = wlc
    assert _find(res, "MERIDIAN-X-WLAN-003", "Secure-WPA3").state == "PASS"
