...
export default defineConfig({
  // The engine serves this app at /dashboard (see meridian/api/app.py), so every
  // asset URL has to carry that prefix. The build writes straight into the
  // package the engine ships, which is what keeps deployment to one command
  // with no separate build step.
  base: '/dashboard/',
  build: {
    outDir: '../meridian/api/static/dashboard',
    emptyOutDir: true,
  },
  plugins: [
    react(),
