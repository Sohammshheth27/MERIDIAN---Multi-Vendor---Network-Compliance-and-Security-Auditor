 ...
 * Console sign-in.
 *
 * Built to design/MERIDIAN_APP_DESIGN_SYSTEM.md: the .meridian-card language, the
...
const Login: FC = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(async (s) => {
        ...
        // MERIDIAN_SHOW_PAIRING=1 keeps it open so the pairing step can be shown
  ...
  const form = (
    <form onSubmit={submit} className="meridian-card p-6 h-full flex flex-col">
      ...
      {status && !status.required && (
        <p className="mt-2 text-[11px] leading-[1.4] text-[var(--color-slate-gray)]">
          Enforcement is off (<code className="font-mono">MERIDIAN_CONSOLE_AUTH=0</code>).
        </p>
      )}
    </form>
  );
...
const Pairing: FC<{ e: AuthEnrollment }> = ({ e }) => (
  <div className="meridian-card p-6 flex flex-col">
    <div className="flex items-center gap-2 mb-1">
