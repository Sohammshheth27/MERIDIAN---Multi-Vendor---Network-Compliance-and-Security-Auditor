...
/** The API token, needed only when the engine runs with MERIDIAN_API_TOKEN set.
...
const ApiAccess: FC = () => {
  ...
  const store = (v: string) => {
    try {
      if (v) localStorage.setItem('meridian_api_token', v);
      else localStorage.removeItem('meridian_api_token');
    } catch {
  ...
  return (
    <Card variant="default" className="p-6">
      ...
      <p className="mb-3 text-[12.5px] text-[var(--color-slate-gray)]">
        Needed only when the engine is started with <code className="font-mono">MERIDIAN_API_TOKEN</code>.
