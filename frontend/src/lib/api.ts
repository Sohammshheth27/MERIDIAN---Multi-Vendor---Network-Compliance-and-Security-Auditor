...
export interface AuthStatus {
   ...
   *  holds it true when started with MERIDIAN_SHOW_PAIRING=1. */
...
/** Governs the model INSIDE MERIDIAN, not the audited device. See /ai-governance. */
...
/** The API token, when the engine requires one (MERIDIAN_API_TOKEN). Kept in this
 *  browser only; storage can be unavailable, so every access is guarded. */
export function apiToken(): string {
  try {
    return localStorage.getItem("meridian_api_token") || "";
  } catch {
    return "";
  }
}
...
const SESSION_USER_KEY = "meridian_session_user";
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(
      0,
      "Cannot reach the Meridian engine. Start it with: uvicorn meridian.api.app:app --port 8000",
    );
  }
...
export const api = {
  ...
  exportFleetCsv: () => download('/fleet.csv', 'meridian_fleet.csv'),
